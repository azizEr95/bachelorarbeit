...
In der Einleitung wurde deutlich, dass \textit{petappoint} eine fokussierte Lösung für die Terminverwaltung zwischen Tierhaltern und Tierarztpraxen darstellt. Die in Kapitel~1 abgeleitete Forschungsfrage zielt darauf ab, eine solche Lösung zu konzipieren, umzusetzen und zu evaluieren. Um diese Lösung zielgerichtet umzusetzen, folgt die Anforderungsermittlung dieser Arbeit den Prinzipien des Requirements Engineering. Requirements Engineering ist eine Disziplin, deren Ziel es ist, die Wünsche und Bedürfnisse aller Stakeholder zu verstehen und zu dokumentieren \cite[S.~3]{pohl2021}. Stakeholder sind wiederum Personen oder Organisationen, die Einfluss auf die Anforderungen des Systems haben \cite[S.~2]{pohl2021}. Dazu zählen auch die Nutzenden von \textit{petappoint}. Eine Anforderung wird dabei als ein notwendiges Bedürfnis eines Stakeholders angesehen, die die Anwendung erfüllen muss \cite[S.~1]{pohl2021}.
...
\section{Zielgruppe}
Die Anwendung richtet sich an zwei primäre Zielgruppen: Tierhalter sowie Tierarztpraxen. Im Rahmen der Analyse bestehender Softwarelösungen in Kapitel 1 wurde deutlich, dass in diesem Bereich keine spezialisierte Terminverwaltungslösung existiert, die beide gleichermaßen adressiert.
...
\begin{itemize}
    \item \textbf{Einfache Bedienbarkeit:} Da Nutzende in der Regel über keine spezifischen technischen Vorkenntnisse verfügen, muss die Anwendung intuitiv bedienbar sein und ohne umfangreiche Einarbeitung genutzt werden können.
...
Zusammenfassend zeigt die Zielgruppenanalyse, dass die Anwendung sowohl für nicht technisch versierte Nutzende auf der Seite der Tierhalter als auch für den professionellen Einsatz in Tierarztpraxen konzipiert werden muss. Die Berücksichtigung beider Perspektiven ist entscheidend für die Akzeptanz und den nachhaltigen Erfolg des Systems.

\section{Funktionale Anforderungen}

Im Requirements Engineering wird grundlegend zwischen funktionalen und nicht-funktionalen Anforderungen unterschieden. Funktionale Anforderungen (FA) beschreiben, welche Funktionen und Fähigkeiten das System den Nutzenden zur Verfügung stellen soll \cite[S.~3]{pohl2021}. Zur eindeutigen Formulierung der funktionalen Anforderungen werden 
...
\begin{itemize}
    \item \textbf{FA\#06} Die mobile Anwendung \textbf{muss} 
...
Die Anforderungen FA\#06 und FA\#07 wurden im Rahmen dieser Arbeit konzipiert, jedoch nicht implementiert. Die Implementierung beschränkt sich auf die Tierhalter-Rolle (FA\#01--FA\#05).
...
\begin{itemize}
    ...
    \item \textbf{NFA\#02} Die mobile Anwendung muss ohne Einarbeitung 
    intuitiv bedienbar sein.

    \item \textbf{NFA\#03} Die mobile Anwendung muss ein helles (Light) sowie ein dunkles (Dark) Farbschema unterstützen, das von Nutzenden manuell über einen Schalter in der Anwendung gewechselt werden kann.

    \item \textbf{NFA\#04} Die mobile Anwendung muss in den Sprachen Deutsch und Englisch verfügbar sein.

    \item \textbf{NFA\#05} Die Anwendung muss Nutzenden durch Statusmeldungen Rückmeldung über abgeschlossene Aktionen geben.
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includesvg[width=0.8\linewidth]{figures/use-case-diagram.svg}
    \caption{Use-Case-Diagramm \textit{petappoint}}
...
12. Der Tierhalter erhält eine Benachrichtigung als 
Buchungsbestätigung. \newline
...
zu „Meine Termine". \newline
9. Der Tierhalter erhält eine Benachrichtigung als 
...
den Termin frei. \newline
7. Der Tierhalter erhält eine Benachrichtigung als 
...
\hline
\textbf{Vorbedingungen} & 
Die Tierarztpraxis ist angemeldet. \\
\hline
...
automatisch per Benachrichtigung. \\
\hline
\textbf{Alternativszenario} &
