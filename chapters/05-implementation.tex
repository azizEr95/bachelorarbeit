...
\subsection{Gemeinsames Typpaket}

Die mobile Anwendung ist Teil eines Monorepos und teilt gemeinsame Typdefinitionen und Validierungsschemata mit dem Backend über das lokale Paket \texttt{petappoint-shared}, welches in der \texttt{tsconfig.json} auf den shared Ordner verweist.

\begin{lstlisting}[captionpos=b, caption={Path Alias für petappoint-shared in tsconfig.json}]
"petappoint-shared/*": [
  "../shared/src/*"
]
\end{lstlisting}

Dadurch ist die konsistente Typisierung über alle Schichten der Anwendung gewährleistet und es werden redundante Definitionen vermieden. 

\section{Umsetzung der Navigationsstruktur mit Expo Router}

Die Navigationsstruktur von petappoint basiert auf Expo Router und wird vollständig über die Ordnerstruktur des \texttt{app}-Verzeichnisses abgebildet. 
\vspace{0.5em}
\dirtree{%
  .1 app/.
  .2 \_layout.tsx.
  ...
  .3 edit-pet.tsx.
  .3 edit-profile.tsx.
  .3 favorite-practices.tsx.
  .3 language.tsx.
}
\vspace{0.5em}
Jede Layout-Datei \texttt{\_layout.tsx} definiert dabei den Navigationscontainer für die jeweilige Routengruppe.
