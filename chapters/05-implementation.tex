...
Aufbauend auf den im vorherigen Kapitel erläuterten technischen Grundlagen beschreibt dieses Kapitel die konkrete Umsetzung der mobilen Anwendung von \textit{petappoint}. Der Fokus liegt dabei ausschließlich auf der Tierhalter-Rolle, da nur diese im Rahmen dieser Arbeit implementiert wurde.
...
Die Navigationsstruktur von \textit{petappoint} basiert auf Expo Router und wird vollständig über die Ordnerstruktur des \texttt{app}-Verzeichnisses abgebildet. Jede Layout-Datei \texttt{\_layout.tsx} definiert dabei den Navigationscontainer für die jeweilige Routengruppe.

\subsection{Root Layout}

Das Root Layout in \texttt{app/\_layout.tsx} bildet den Einstiegspunkt der gesamten Anwendung und entspricht konzeptionell der \texttt{App.tsx} in einer klassischen React-Anwendung.Es stellt globale Provider und Konfigurationen bereit und initialisiert diese für alle untergeordneten Komponenten, bevor die eigentliche Navigation gestartet wird. Diese werden in den nächsten Abschnitten nochmal detaillierter erklärt.
...
...
\end{minted}
\caption{Registrierung der Routengruppen und der Modal Screens im Root-Stack}
\label{lst:root-stack}
\end{listing}
...
\end{minted}
\caption{Authentication Stack in app/(auth)/\_layout.tsx}
\label{lst:auth-layout}
\end{listing}
...
/>
\end{minted}
\caption{Definition eines Tab-Screens mit Icon und Übersetzung}
\label{lst:tab-screen}
\end{listing}
...
Die Authentifizierung in \textit{petappoint} gliedert sich in vier Bereiche: die sichere Speicherung des Zugriffstokens, die Verwaltung des Authentifizierungszustands zur Laufzeit, die automatische Token-Übermittlung bei API-Aufrufen sowie die Validierung einer bestehenden Sitzung beim Backend.
...
\end{minted}
\caption{Plattformabhängiger Storage-Wrapper in authStore.ts}
\label{lst:storage-wrapper}
\end{listing}
...
\end{minted}
\caption{Hydration des Authentifizierungszustands beim App-Start}
\label{lst:hydrate}
\end{listing}
...
\end{minted}
\caption{Speichern des Tokens und Nutzerobjekt nach erfolgreicher Anmeldung}
\label{lst:set-auth}
\end{listing}
...
\end{minted}
\caption{Löschen des Tokens und Nutzerobjekt bei Abmeldung}
\label{lst:clear-auth}
\end{listing}
...
\end{minted}
\caption{Verified-Flag im Nutzerobjekt setzen}
\label{lst:set-verified}
\end{listing}
...
\end{minted}
\caption{Token-Injection und 401-Handling in apiRequest}
\label{lst:api-request}
\end{listing}

Zweitens behandelt \texttt{apiRequest} HTTP-401-Antworten zentral: Gibt das Backend einen 401-Statuscode zurück wird \texttt{clearAuth()} automatisch aufgerufen. Der Token wird aus dem Store entfernt und leitet den Nutzenden beim nächsten Render automatisch zum Login-Screen weiter.
...
\end{minted}
\caption{Login-Flow in useLogin.ts}
\label{lst:use-login}
\end{listing}
...
\end{minted}
\caption{Logout-Flow in useLogout.ts}
\label{lst:use-logout}
\end{listing}
...
\end{minted}
\caption{useSession Hook}
\label{lst:use-session}
\end{listing}

Mit \texttt{retry: false} wird verhindert, dass TanStack Query den Aufruf bei einem Fehler
wiederholt. Das heißt, ein ungültiges Token führt sofort zur Abmeldung. Die \texttt{staleTime}
...
\end{minted}
\caption{QueryClient-Konfiguration in QueryProvider.tsx}
\label{lst:query-client}
\end{listing}
...
\end{minted}
\caption{Einfacher Query-Hook am Beispiel useAllPractices}
\label{lst:use-all-practices}
\end{listing}
...
\end{minted}
\caption{Parallele Queries in usePracticeDetails}
\label{lst:use-practice-details}
\end{listing}
...
'favorites'      -> [userId]
\end{minted}
\caption{Query-Key-Hierarchie der Anwendung}
\label{lst:query-keys}
\end{listing}
...
\end{minted}
\caption{Cache-Invalidierung nach Stornierung in useCancelAppointment}
\label{lst:cancel-appointment}
\end{listing}
...
\end{minted}
\caption{Aufbau des Home Screens}
\label{lst:home-screen}
\end{listing}
...
\end{minted}
\caption{Farbtoken für Light- und Dark-Mode in config.ts}
\label{lst:color-tokens}
\end{listing}
...
\end{minted}
\caption{Zentralisierte Theme-Logik im FontAwesomeIcon-Wrapper}
\label{lst:fontawesome-wrapper}
\end{listing}
...
\end{minted}
\caption{Explizite Farbangabe bei abweichenden Icon-Farben}
\label{lst:icon-color}
\end{listing}
...
\subsection{Übersicht der implementierten Screens}

Die implementierten Screens gliedern sich in drei Bereiche: den Authentifizierungsbereich, die Tab-Navigation sowie den Buchungsprozess. Die vollständigen Screenshots der einzelnen Screens sind im Anhang unter Abbildung~\ref{fig:anhang_authentication}, \ref{fig:anhang_tab-light}, \ref{fig:anhang_tab-dark} und \ref{fig:anhang_booking-process} abgebildet.

Der Authentifizierungsbereich umfasst die Screens für Login, Registrierung, E-Mail-Verifizierung sowie Passwort-vergessen (siehe Anhang, Abbildung~\ref{fig:anhang_authentication}).

Die Tab-Navigation besteht aus den vier Hauptbereichen Home, Termine, Haustiere und Profil. Alle Screens unterstützen sowohl das Light- als auch das Dark-Theme. Im Anhang ist ergänzend das Dark-Theme dargestellt (siehe Anhang, Abbildungen~\ref{fig:anhang_tab-light} und \ref{fig:anhang_tab-dark}).

Der Buchungsprozess ist als modaler Ablauf umgesetzt und führt den Tierhalter schrittweise durch die Suche nach einer Tierarztpraxis, die Auswahl einer Leistung sowie eines freien Termins bis hin zur Buchungsbestätigung (siehe Anhang, Abbildung~\ref{fig:anhang_booking-process}).